...
model Tank
  ...
  //  Real L "Level in %";
  //protected
  parameter Boolean use_p_top = false "= true to get p_top from an input connector" annotation(
    Dialog(group = "Assumptions"),
    Evaluate = true,
    HideResult = true,
    choices(checkBox = true));
  parameter SI.AbsolutePressure p_fixed = Medium.p_default "Fixed value of pressure" annotation(
    Evaluate = true,
  ...
  parameter Boolean enable_losses = false "= true enable thermal losses with environment" annotation(
    ...
    HideResult = true,
    choices(checkBox = true));
  parameter SI.CoefficientOfHeatTransfer alpha = 1 "Constant heat transfer coefficient with the ambient" annotation(
    Dialog(group = "Assumptions", enable = enable_losses));
  parameter Boolean use_L = false "= true to display level as output variable" annotation(
    Dialog(group = "Operating strategy"),
    Evaluate = true,
    HideResult = true,
    choices(checkBox = true));
  parameter Real L_start = 70 "Start value of level in %" annotation(
    Dialog(group = "Initialization"));
  parameter SI.Temperature T_start = from_degC(500) "Start value of temperature" annotation(
    Dialog(group = "Initialization"));
  parameter SI.Temperature T_set = from_degC(500) "Tank Heater Temperature Set-Point" annotation(
    Dialog(group = "Heater"));
  parameter SI.Power W_max = 30e8 "Hot Tank Heater Capacity" annotation(
    Dialog(group = "Heater"));
  parameter SI.Efficiency e_ht = 0.99 "Tank Heater Efficiency" annotation(
    Dialog(group = "Heater"));
  Modelica.Blocks.Interfaces.RealOutput h_fluid = medium.h "Enthalpy of fluid" annotation(
    Placement(visible = true, transformation(origin = {107, 3}, extent = {{-11, -11}, {11, 11}}, rotation = 0), iconTransformation(origin = {102, -10}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  SI.Volume V;
  SI.Mass m;
  ...
  SI.Power W_net;
  SI.Power W_loss;
  Modelica.Blocks.Interfaces.RealOutput L if use_L "Tank level in %" annotation(
    Placement(transformation(extent = {{96, 44}, {116, 64}}), iconTransformation(extent = {{92, 34}, {112, 54}})));
  Modelica.Blocks.Interfaces.RealInput p_top if use_p_top annotation(
  ...
  Modelica.Blocks.Interfaces.RealInput T_amb if enable_losses annotation(
    Placement(visible = true, transformation(origin = {-34, 104}, extent = {{-16, -16}, {16, 16}}, rotation = -90), iconTransformation(origin = {-41, 97}, extent = {{-11, -11}, {11, 11}}, rotation = -90)));
protected
